  ...
  useEffect(() => {
    const loadSound = async () => {
      try {
        ...
        const response = await fetch(sitePath('/assets/keycap-sounds/press.mp3'));
        const arrayBuffer = await response.arrayBuffer();
        const decodedBuffer = await ctx.decodeAudioData(arrayBuffer);
        pressBufferRef.current = decodedBuffer;

        const releaseResponse = await fetch(sitePath('/assets/keycap-sounds/release.mp3'));
        ...
        const confettiResponse = await fetch(sitePath('/assets/sounds/vine-boom.mp3'));
